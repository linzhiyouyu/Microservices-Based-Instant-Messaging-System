...
public:
    ...
    bool registry(const std::string& key, const std::string& val) {
        auto resp =  client.put("/service/user", "127.0.0.1:8081", lease_id).get();
        if(resp.is_ok() == false) {
...
public:
        ...
        : _client(std::make_shared<etcd::Client>(host)), _put_cb(put_cb), _del_cb(del_cb)
    {
        ...
        //1. 进行服务发现
        auto resp = client.ls(basedir).get();
        if(resp.is_ok() == false) {
            ERROR("获取服务信息数据失败: {}", resp.error_message());
        }
        ...
        _watcher = std::make_shared<etcd::Watcher>(*_client.get(), basedir, std::bind(&Discovery::callback, std::place_holders::_1), true);
    }
